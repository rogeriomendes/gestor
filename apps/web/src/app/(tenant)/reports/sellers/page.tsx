...
export default function SellersReportPage() {
  const report = getReportById("sellers");
  ...
  const { data, isLoading, isError } = useQuery({
    ...trpc.tenant.reports.salesPerSeller.queryOptions({
  ...
  const getExportData = () => {
    return report.getExportData({ salesPerSeller: data });
  };
    ...
    >
      <div className="space-y-6">
        {isLoading ? (
          ...
          <SellersCharts salesPerSeller={data} />
        )}
      </div>
    </PageLayout>
  );
