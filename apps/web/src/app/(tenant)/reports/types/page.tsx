...
export default function TypesReportPage() {
  const report = getReportById("types");
  ...
  const { data, isLoading, isError } = useQuery({
    ...trpc.tenant.reports.salesPerType.queryOptions({
  ...
  const getExportData = () => {
    return report.getExportData({ salesPerType: data });
  };
    ...
    >
      <div className="space-y-6">
        {isLoading ? (
          ...
          <TypesCharts salesPerType={data as any} />
        )}
      </div>
    </PageLayout>
  );
