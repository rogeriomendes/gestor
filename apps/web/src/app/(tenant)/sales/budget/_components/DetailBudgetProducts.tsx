...
export function DetailBudgetProducts({
  budgetId,
}: {
  budgetId: number | undefined;
}) {
  const { tenant } = useTenant();
  const budgetProductsByIdQuery = useQuery({
    ...trpc.tenant.salesBudget.productsById.queryOptions({ id: budgetId }),
    enabled: !!tenant,
  });

  if (budgetProductsByIdQuery.isPending) {
    return (
  ...
  return (
    <>
      {budgetProductsByIdQuery.data?.budget?.map((products: any) => (
        <div
